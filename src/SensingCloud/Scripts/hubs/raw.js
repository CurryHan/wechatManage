...
$(function () {
    var connection = $.connection("/Connections/Raw");
    ...
    $("#sendToMe").click(function () {
        connection.send({ type: "sendToMe", content: $("#message").val() , action:$("#action").val()})
    });

    $("#sendToConnectionId").click(function () {
        connection.send({ type: "sendToConnectionId", content: $("#message").val(), connectionId: $("#connectionId").val(), action: $("#action").val() });
    });

    $("#sendBroadcast").click(function () {
        connection.send({ type: "sendBroadcast", content: $("#message").val(), action: $("#action").val() });
    });

    $("#sendToGroup").click(function () {
        connection.send({ type: "sendToGroup", content: $("#message").val(), groupName: $("#groupName").val(), action: $("#action").val() });
    });

    $("#joinGroup").click(function () {
        connection.send({ type: "joinGroup", groupName: $("#groupName").val(), connectionId: $("#connectionId").val(), action: $("#action").val() });
    });

    $("#leaveGroup").click(function () {
        connection.send({ type: "leaveGroup", groupName: $("#groupName").val(), connectionId: $("#connectionId").val(), action: $("#action").val() });
    });

    $("#throw").click(function () {
        connection.send({ type: "throw", action: $("#action").val() });
    });
});
