    ...
    internal class CommonConventions
    {
            ...
            foreach (var interfaceType in interfaces)
            {
                if (!IsExcludedType(interfaceType, excludeTypes, excludeRegEx))
                {
                    ...
                    Type implementationType =
                        implementations.Where(implementation => IsDefaultType(interfaceType, implementation))
                            .FirstOrDefault();
                    ...
                    if (implementationType == null)
                    {
                        ...
                        implementationType =
                            implementations.Where(implementation => IsAdapterType(interfaceType, implementation))
                                .FirstOrDefault();
                    }

                    if (implementationType != null)
                    {
                        System.Diagnostics.Debug.WriteLine("Auto registration of {1} : {0}", interfaceType.Name,
            ...
            string excludeRegEx)
        {
            foreach (var interfaceType in interfaceTypes)
            {
                ...
                System.Diagnostics.Debug.WriteLine("Auto multiple registration of {1} : {0}", interfaceType.Name,
        ...
        {
            return
                type.GetCustomAttributes(typeof (MvcSiteMapProvider.DI.ExcludeFromAutoRegistrationAttribute), false)
                    .Length > 0;
        }
        ...
        private static bool IsAdapterType(Type interfaceType, Type implementationType)
        {
            return implementationType.Name.EndsWith("Adapter") &&
                   interfaceType.Name.Equals("I" +
                                             implementationType.Name.Substring(0, implementationType.Name.Length - 7));
        }
